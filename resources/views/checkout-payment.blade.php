    ...
    <div class="checkout-container">
        <div class="checkout-card">
            ...
            <div class="product-summary">
                <h1>{{ $price->type ?? 'Dịch vụ' }} - {{ $price->hours_label }}</h1>
                <div class="price-row">
                    <div>
                        <div class="price-label">TỔNG THANH TOÁN</div>
                    </div>
                    <div class="price-value">{{ number_format($order->amount, 0, ',', '.') }}<small>đ</small></div>
                </div>
                <div class="tracking-badge">
                    Mã đơn: <strong>{{ $order->tracking_code }}</strong>
                </div>
            </div>
                ...
                <div class="payment-left">
                    <div class="qr-section">
                        ...
                        <div class="qr-wrapper">
                            <img src="{{ $qrUrl }}" alt="QR Payment" class="qr-img">
                        </div>
                        <div class="bank-info">
                            <div class="bank-row">
                                <span>Ngân hàng:</span>
                            ...
                            <div class="bank-row">
                                <span>Nội dung:</span>
                                <strong class="tracking-code">{{ $order->tracking_code }}</strong>
                            </div>
                            <div class="bank-row">
                                <span>Số tiền:</span>
                                <strong class="amount">{{ number_format($order->amount, 0, ',', '.') }} đ</strong>
                            </div>
                        </div>
                        <div class="payment-warning">
                            ⚠️ Nội dung CK phải giống 100% "<strong>{{ $order->tracking_code }}</strong>"
                        </div>
                    </div>
                </div>
                ...
                <div class="payment-right">
                    <div class="order-summary-box">
                        ...
                        <div class="summary-row">
                            <span>Dịch vụ</span>
                            <span class="summary-value">{{ $price->type }}</span>
                        </div>
                        <div class="summary-row">
                            <span>Thời hạn</span>
                            <span class="summary-value">{{ $price->hours_label }}</span>
                        </div>
                        ...
                        <div class="balance-section">
                            <div class="balance-row">
                                ...
                                <span class="balance-amt">{{ number_format($userBalance, 0, ',', '.') }} đ</span>
                            </div>
                            ...
                            @else
                            <div class="balance-warning">⚠️ Thiếu {{ number_format($order->amount - $userBalance, 0, ',', '.') }} đ</div>
        ...
        fetch('/api/check-payment?code=' + encodeURIComponent(trackingCode))
            ...
            .then(data => {
                ...
                if (data.paid) {
                    ...
                    document.getElementById('status-checking').style.background = '#d1fae5';
                    setTimeout(() => {
        ...
        payBtn.addEventListener('click', function() {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    payBtn.textContent = 'Thanh toán bằng số dư';
                }
            })
            .catch(err => {
